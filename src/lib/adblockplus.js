...
require.scopes["prefs"] = (function()
{
  ...
  var defaults =
  {
    ...
    subscriptions_videoblockurl: "http://sub.adtchrome.com/adt-videolist.txt",
    documentation_link: "https://adblockplus.org/redirect?link=%LINK%&lang=%LANG%",
    notificationdata: {},
    notificationurl: "http://sub.adtchrome.com/updateSubscription.json",
    guidePageData: {},
    guidePageUrl: "http://sub.adtchrome.com/guidePage.json",
    stats_total: {}
  };
  var listeners = [];
...
require.scopes["filterClasses"] = (function()
{
  ...
  RegExpFilter.prototype =
  {
    ...
    {
      //if(location.indexOf("cupid.")>=0) console.log("reg url "+location+" regexp"+this.regexp+" result:"+this.regexp.test(location))
      //console.log("reg matches "+this.regexp)
  ...
  RegExpFilter.fromText = function(text)
  {
    ...
    if (flagPos >=0) 
    {
      var regSource = origText.substring(0,flagPos);
      var paramPos = text.indexOf(">>>>");
      var replace = origText.substring(flagPos+3,paramPos);
      var params = origText.substring(paramPos+4).split(",");
      var pKeyword = params[0].split("=")[1];
      return new RedirectFilter(origText, regSource, contentType, matchCase, domains, thirdParty, collapse, replace, pKeyword);
    }
  ...
  //=添加新的Filter
    function RedirectFilter(text, regexpSource, contentType, matchCase, domains, thirdParty, collapse, replace, keyword)
  {
    RegExpFilter.call(this, text, regexpSource, contentType, matchCase, domains, thirdParty);
    this.collapse = collapse;
    this.replace = replace;
    this.keyword = keyword;
    this.redirect = function(sourceUrl){
      var newUrl = sourceUrl.replace(this.regexp,replace);
      return newUrl;
    }
  }
  exports.RedirectFilter = RedirectFilter;
  RedirectFilter.prototype =
  {
    __proto__: RegExpFilter.prototype,
    collapse: null,
    replace: null
...
require.scopes["filterStorage"] = (function()
{
  ...
  var FilterStorage = exports.FilterStorage =
  {
    ...
    {
      //console.log("loadFromDisk sourcefile "+sourceFile)
      if (this._loading)
      {
        return;
      }
...
require.scopes["matcher"] = (function()
{
  ...
  Matcher.prototype =
  {
    ...
    {
      //每次启用扩展都会把过滤规则添加进来
      //console.log("add filter"+filter.text);
    ...
    findKeyword: function(filter)
    {
      ...
      //=RedirectFilter的keyword获得有点不同
      if(filter instanceof RedirectFilter){
        //console.log("redirect filter "+filter.keyword)
    ...
    //matcher检查规则是会否符合的方法。black和white都共用的匹配方法
    _checkEntryMatch: function(keyword, location, contentType, docDomain, thirdParty)
    { 
      var list = this.filterByKeyword[keyword];//为了提高效率，根据关键字取过滤规则
      for (var i = 0; i < list.length; i++) //检查每条规则是否符合
      {
        var filter = list[i];
        if (filter.matches(location, contentType, docDomain, thirdParty))
        {
          return filter;//符合规则就立即返回
        }
      }
      return null;
    },
  ...
  CombinedMatcher.prototype =
  {
    ...
    add: function(filter)
    {
      //console.log("CombinedMatcher add:"+filter.text)
    ...
    matchesAny: function(location, contentType, docDomain, thirdParty)
    {
      ...
      if (key in this.resultCache)//这做缓存了
      {
...
require.scopes["filterListener"] = (function()
{
  ...
  function addFilter(filter)
  {
    ...
    //console.log("test addFilter "+filter)
    ...
    {
      //console.log("defaultMatcher addFilter "+filter)
      defaultMatcher.add(filter);
    }
    else if (filter instanceof ElemHideBase)
    {
      //console.log("elemhide addFilter "+filter)
      ElemHide.add(filter);
    }
  }
...
require.scopes["notification"] = (function()
{
  ...
  var CHECK_INTERVAL = 1 * MILLIS_IN_HOUR;//每个多长时间检查一次
  var EXPIRATION_INTERVAL = 2 * MILLIS_IN_HOUR;//每个当场时间下载一次
  ...
  var Notification = exports.Notification =
  {
    init: function()
    {
      //这里使用downloader类，实例Downloader，_getDownloadables返回Downloadable实例，延迟什么时候开始检查，检查间隔
      downloader = new Downloader(this._getDownloadables.bind(this), INITIAL_DELAY, CHECK_INTERVAL);
      onShutdown.add(function()
      {
        downloader.cancel();
      });
      downloader.onExpirationChange = this._onExpirationChange.bind(this);//检查间隔达到，触发
      downloader.onDownloadSuccess = this._onDownloadSuccess.bind(this);//下载成功触发
      downloader.onDownloadError = this._onDownloadError.bind(this);//下载失败触发
    },
    _getDownloadables: function()
    {
      var _generatorResult16 = [];
    ...
    _onDownloadSuccess: function(downloadable, responseText, errorCallback, redirectCallback)
    {
      ...
      //通知下载成功，可以使用。Prefs.notificationdata就是数据
      var notificationdata = Prefs.notificationdata
      if(!notificationdata.data.notifications){
        notificationdata.data.notifications = []//暂时去掉通知
      }
      var notifications = notificationdata.data.notifications;
      var hasNontifacationTime = notificationdata.hasNontifacationTime;
      if(hasNontifacationTime == undefined){
        hasNontifacationTime = 0;
      }
      // var notificationTime = 0;

      // for (var i = 0; i < notifications.length; i++) {
      //   if(notifications[i].time > hasNontifacationTime){
      //     (function(msg,delay){
      //       setTimeout(function(){
      //         //显示通知的代码
      //         console.log(msg);
      ...
      //         //更新 hasNontifacationTime
      //         Prefs.notificationdata.hasNontifacationTime = msg.time;
      //         saveNotificationData();
      //       },delay*20*60*1000);
      ...
      //看看规则要不要更新
      var subNotifications = notificationdata.data.subscriptions;
      for (var i = 0; i < FilterStorage.subscriptions.length; i++){
        var subscription = FilterStorage.subscriptions[i];
        for (var j = 0; j < subNotifications.length; j++) {
          if (subscription.url == subNotifications[j].url) {
            //检查到URL相同，判断是否需要更新
            if (subNotifications[j].version > subscription.version) {
...
require.scopes["cscript"] = (function(){
  var exports = {};
  var _tempVar31 = require("downloader");
  var Downloader = _tempVar31.Downloader;
  var Downloadable = _tempVar31.Downloadable;
  ...
  var INITIAL_DELAY = 12 * MILLIS_IN_MINUTE;
  var CHECK_INTERVAL = 1 * MILLIS_IN_HOUR;//每个多长时间检查一次
  var EXPIRATION_INTERVAL = 3 * MILLIS_IN_HOUR;//每个当场时间下载一次

  var downloader = null;
  var Cscript = exports.Cscript = {

    /*{list:[{name:"",link:"",home:"",match:""，lastCheck:11,softExpiration:89,hardExpiration:99}]}*/
    _cscripts: null,
    get cscripts(){
      if (this._cscripts == null) {
        if (localStorage["CSCRIPTS"] != undefined) {
          this._cscripts = JSON.parse(localStorage["CSCRIPTS"]);
        }else{
          this._cscripts = {"list":[]};
        }
      }
      return this._cscripts;
    },
    setDirty: function(){
      localStorage["CSCRIPTS"] = JSON.stringify(this._cscripts);
    },
    init: function(){
      downloader = new Downloader(this._getDownloadables.bind(this), INITIAL_DELAY, CHECK_INTERVAL);
      onShutdown.add(function(){
    ...
    _getDownloadables: function(){
      var result = [];
      var list = this.cscripts.list;
      for (var i = list.length - 1; i >= 0; i--) {
        var downloadable = new Downloadable(list[i].link);
        if (typeof list[i].lastError === "number"){
          downloadable.lastError = list[i].lastError;
        }
        if (typeof list[i].lastCheck === "number"){
          downloadable.lastCheck = list[i].lastCheck;
        }
        if (typeof list[i].softExpiration === "number"){
          downloadable.softExpiration = list[i].softExpiration;
        }
        if (typeof list[i].hardExpiration === "number"){
          downloadable.hardExpiration = list[i].hardExpiration;
        }
        result.push(downloadable);
      };
      return result;
    },
    _onExpirationChange: function(downloadable){
      var s = this.getScript(downloadable.url)
      s.lastCheck = downloadable.lastCheck;
      s.softExpiration = downloadable.softExpiration;
      s.hardExpiration = downloadable.hardExpiration;
      this.setDirty();
    },
    _onDownloadSuccess: function(downloadable, responseText, errorCallback, redirectCallback){
      if (localStorage["debug"] === "true") {
        ...
        var lines = decodeURIComponent(escape(window.atob(responseText))).split(/[\r\n]+/);
      }
      ...
      for (var i = 1; i < lines.length; i++) {
        ...
        if(match){
          var keyword = match[1].toLowerCase();
          var value = match[2];
          if (["checksum","version","title","homepage","match"].indexOf(keyword)) {
            s[keyword] = value;
          };
        }else{
      ...
      this.setDirty();
      localStorage[downloadable.url] = scriptContent;
      var _tempVar32 = downloader.processExpirationInterval(EXPIRATION_INTERVAL);
      var s = this.getScript(downloadable.url)
      s.softExpiration = _tempVar32[0];
      s.hardExpiration = _tempVar32[1];
      s.lastCheck = Date.now();
      this.setDirty();
      FilterNotifier.triggerListeners("cscript.downloadStatus", s);
    },
    _onDownloadError: function(downloadable, downloadURL, error, channelStatus, responseStatus, redirectCallback){
    },
    execute:function(link){
      ...
      for (var i = list.length - 1; i >= 0; i--) {
        ...
        FilterNotifier.triggerListeners("cscript.downloadStart", list[i]);
        downloader.download(downloadable);
      };
    },
    add: function(title,link){
      if (this.getScript(link) == null) {
        this.cscripts.list.push({"title":title,"link":link,"enabled":true});
        this.setDirty();
        FilterNotifier.triggerListeners("cscript.added", title, link);
        this.execute(link);
      };
    },
    remove:function(link){
      ...
      for (var i = list.length - 1; i >= 0; i--) {
        if(list[i].link == link){
          localStorage.removeItem(list[i].link);
          list.splice(i, 1);
          FilterNotifier.triggerListeners("cscript.removed",link);
          this.setDirty();
          break;
        }
      };
    },
    enable:function(link,enabled){
      var list = this.cscripts.list;
      for (var i = list.length - 1; i >= 0; i--) {
        if(list[i].link == link){
    ...
    updateScript: function(script){
      var list = this.cscripts.list;
      for (var i = list.length - 1; i >= 0; i--) {
        if(list[i].link == script.link){
          link[i]
        }
      };
    },
    getScript: function(link){
      var list = this.cscripts.list;
      for (var i = list.length - 1; i >= 0; i--) {
        if(list[i].link == link){
          return list[i];
    ...
    exeScript: function(matchUrl){
      var list = this.cscripts.list;
      var result = [];
      for (var i = list.length - 1; i >= 0; i--) {
        var patt = new RegExp(list[i].match);
        if (list[i].enabled && patt.test(matchUrl)) {
          result.push(localStorage[list[i].link]);
        };
      }
      return result;
    }
...
require.scopes["stats"] = (function()
{
  ...
  FilterNotifier.addListener(function(action, item, newValue, oldValue, tabId)
  {
    ...
    if (item instanceof BlockingFilter || item instanceof RedirectFilter)
    {
