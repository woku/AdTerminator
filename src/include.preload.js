...
function setElemhideCSSRules(selectors)
{
  ...
  elemhideElt.setAttribute("abt", "234");
...
function checkCollapse(event)
{
  ...
  if (tag in typeMap && event.type == expectedEvent)
  {
    ...
    chrome.runtime.sendMessage({reqtype: "should-collapse", url: url, documentUrl: document.URL, type: type}, function(response)
    {
      if(Object.keys(response).length === 0 && response.constructor === Object) response = false;//修复iframe消失的bug，原因浏览器抢着返回空object
...
function init()
{
  ...
  chrome.runtime.sendMessage({reqtype: "get-settings", selectors: true, frameUrl: window.location.href}, function(response)
  {
